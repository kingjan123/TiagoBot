...
local wikiusage = {
  ...
  "!wiki(lang) search [text]: جستجو مطلب در ویکی پدیای غیر فارسی مانند:!wikien apple",
}
...
language = "fa"
  ...
  if result and result.query then
    ...
    if page and page.extract then
      return text..": "..page.extract.."\nادامه را در ویکی پدیا بخوانید:\n https://"..language..".wikipedia.org/wiki/"..URL.escape(text)
    else
      local text = " هیچ نتیجه ای برای"..text.."یافت نشد "
      text = text..'\n'..table.concat(wikiusage, '\n')
      return text
    end
  else
    return "متاسفانه مشکلی پیش آمد"
  end
end
...
function Wikipedia:wikisearch(text, lang)
  ...
  if result and result.query then
    local titles = ""
	 ...
	 titles = titles ~= "" and titles or "هیچ نتیجه ای یافت نشد"
	 return titles
  else
    return "متاسفانه مشکلی پیش آمد"
  end

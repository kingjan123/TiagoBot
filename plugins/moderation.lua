...
local function mute_by_reply(extra, success, result)
    ...
    local user = msg.from.id
    local name = msg.from.username
	if permissionst(sender, user, chat) then
    local hash = 'muted:'..chat..':'..user
    redis:set(hash, true)
    if msg.to.type == 'chat' then
   ...
   if msg.to.type == 'chat' then
        send_msg('chat#id'..chat, '🚫 '..lang_text(chat, 'require_up'), ok_cb, false)
    elseif msg.to.type == 'channel' then
        send_msg('channel#id'..chat, '🚫 '..lang_text(chat, 'require_up'), ok_cb, false)
    end
	end
end
...
local function mute_by_username(cb_extra, success, result)
    ...
    elseif chat_type == 'channel' then
	    ...
	    if chat_type == 'chat' then
        send_msg('chat#id'..chat_id, '🚫 '..lang_text(chat_id, 'require_up'), ok_cb, false)
    elseif chat_type == 'channel' then
        send_msg('channel#id'..chat_id, '🚫 '..lang_text(chat_id, 'require_up'), ok_cb, false)
    end
	end
end
...
local function chat_kick(extra, success, result)
    ...
    elseif chat_type == 'channel' then
        ...
        send_msg('channel#id'..chat, 'ℹ️ '..lang_text(chat, 'kickUser:1')..' '..user..' '..lang_text(chat, 'kickUser:2'), ok_cb,  true)
		    ...
		    if chat_type == 'chat' then
        send_msg('chat#id'..chat_id, '🚫 '..lang_text(chat, 'require_up'), ok_cb, false)
    elseif chat_type == 'channel' then
        send_msg('channel#id'..chat_id, '🚫 '..lang_text(chat, 'require_up'), ok_cb, false)
    end
	end
end
...
local function channel_ban(extra, success, result)
    ...
    local user = msg.from.id
	if permissionst(sender, user, chat) then
    channel_kick('channel#id'..chat, 'user#id'..user, ok_cb, true)
...
local function run(msg, matches)
    ...
    if matches[1] == 'ban' then
        if permissions(msg.from.id, msg.to.id, "ban") then
            ...
            if msg.reply_id then
                if msg.to.type == 'chat' then
                    get_message(msg.reply_id, chat_ban, {sender=msg.from.id})
                elseif msg.to.type == 'channel' then
                    get_message(msg.reply_id, channel_ban, {sender=msg.from.id})
                end
                return
            end
            if not is_id(matches[2]) then
                local member = string.gsub(matches[2], '@', '')
                resolve_username(member, ban_by_username, {chat_id=chat_id, sender=msg.from.id, member=member, chat_type=chat_type})
                return
            else
                user_id = matches[2]
				if permissionst(msg.from.id, user_id, chat_id) then
                if chat_type == 'chat' then
                    send_msg('chat#id'..chat_id, 'ℹ️ '..lang_text(chat_id, 'banUser:1')..' '..user_id..' '..lang_text(chat_id, 'banUser:2'), ok_cb, false)
                    chat_del_user('chat#id'..chat_id, 'user#id'..user_id, ok_cb, false)
                elseif chat_type == 'channel' then
                    send_msg('channel#id'..chat_id, 'ℹ️ '..lang_text(chat_id, 'banUser:1')..' '..user_id..' '..lang_text(chat_id, 'banUser:2'), ok_cb, false)
                    channel_kick('channel#id'..chat_id, 'user#id'..user_id, ok_cb, false)
                end
				else
				return '🚫 '..lang_text(msg.to.id, 'require_up')
                end
                ban_user(user_id, chat_id)
                return
            end
    ...
    elseif matches[1] == 'kick' then
        if permissions(msg.from.id, msg.to.id, "kick") then
            ...
            -- Using pattern #kick
            if msg.reply_id then
                get_message(msg.reply_id, chat_kick, {sender=msg.from.id})
                return
            end
            if not is_id(matches[2]) then
                local member = string.gsub(matches[2], '@', '')
                resolve_username(member, kick_by_username, {chat_id=chat_id, sender=msg.from.id, member=member, chat_type=chat_type})
                return
            else
    ...
    elseif matches[1] == 'gban' then
        if permissions(msg.from.id, msg.to.id, "gban") then
            ...
            if msg.reply_id then
                get_message(msg.reply_id, gban_by_reply, {sender=msg.from.id})
                return
            end
            if not is_id(matches[2]) then
                local member = string.gsub(matches[2], '@', '')
                resolve_username(member, gban_by_username, {chat_id=chat_id, sender=msg.from.id, member=member, chat_type=chat_type})
                return
            else
                local user_id = matches[2]
				if permissionst(msg.from.id, user_id, chat_id) then
    ...
    elseif matches[1] == 'mute' then
        if permissions(msg.from.id, msg.to.id, "mute") then
            if msg.reply_id then
                get_message(msg.reply_id, mute_by_reply, {sender=msg.from.id})
                return
            end
            if matches[2] then
                if is_id(matches[2]) then
				if permissionst(msg.from.id, matches[2], chat_id) then
                ...
                return '🚫 '..lang_text(msg.to.id, 'require_up')
                    ...
                    local chat_id = msg.to.id
                    local chat_type = msg.to.type
                    resolve_username(member, mute_by_username, {chat_id=chat_id, sender=msg.from.id, member=member, chat_type=chat_type})
                    return
                end
            end
        else
